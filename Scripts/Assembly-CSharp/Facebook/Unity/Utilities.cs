...
namespace Facebook.Unity
{
	internal static class Utilities
	{
		...
		public static string FormatToString(string baseString, string className, IDictionary<string, string> propertiesAndValues)
		{
			...
			foreach (KeyValuePair<string, string> propertiesAndValue in propertiesAndValues)
			{
		...
		private static DateTime ParseExpirationDateFromResult(IDictionary<string, object> resultDictionary)
		{
			if (Constants.IsWeb)
			{
				long valueOrDefault = resultDictionary.GetValueOrDefault<long>(LoginResult.ExpirationTimestampKey);
				return DateTime.UtcNow.AddSeconds(valueOrDefault);
			}
			string valueOrDefault2 = resultDictionary.GetValueOrDefault<string>(LoginResult.ExpirationTimestampKey);
			int result;
