				...
				else
				{
					...
					FB.API("/me/permissions?access_token=" + accessToken, HttpMethod.GET, delegate(IGraphResult permResult)
					{
						...
						else
						{
							...
							AccessToken accessToken = new AccessToken(this.accessToken, facebookID, DateTime.UtcNow.AddDays(60.0), list, DateTime.UtcNow);
							...
							{
								base.Callback(new ResultContainer(dictionary2));
							}
						}
					});
				}
